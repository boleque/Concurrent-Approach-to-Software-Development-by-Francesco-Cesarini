...
% UPD new handlers
swap_handlers(Name, NewHandler, OldHandler) ->
  call(Name, {swap_handlers, NewHandler, OldHandler}).
...
% UPD new msg handler to support swap_handler
handle_msg({swap_handlers, NewHandler, OldHandler}, LoopData) ->
  case lists:keysearch(OldHandler, 1, LoopData) of
    false -> 
      {{error, instance}, LoopData};
    {value, {OldHandler, Data}} ->
      OldHandlerData = OldHandler:terminate(Data),
      NewLoopData = [
          NewHandler:init(OldHandlerData) |
